...
fn oa_bot_for_agent(home_dir: &std::path::Path, agent_name: &str) -> String {
    carrier_types::config::scan_sender_sessions(home_dir)
        .into_iter()
        .find(|(_, json)| {
...
pub(super) async fn cron_fire_job(kernel: &Arc<CarrierKernel>, job: CronJob) -> Result<(), String> {
    ...
    match &job.action {
        ...
        carrier_types::scheduler::CronAction::FollowerReport { channel, bot_id } => {
        ...
        carrier_types::scheduler::CronAction::PublishPoll { channel: _, bot_id } => {
            tracing::info!(job = %job_name, "Cron: firing publish poll (no LLM)");
            let home = kernel.config.home_dir.clone();
            let pending_ids = carrier_wechat_oa::publish_tracker::pending(&home, bot_id);
            if pending_ids.is_empty() {
                let streak =
                    carrier_wechat_oa::publish_tracker::bump_streak(&home, bot_id).unwrap_or_default();
                if streak >= 3 {
                    let _ = kernel.cron_scheduler.remove_job(job_id);
                    let _ = kernel.cron_scheduler.persist();
            ...
            let Some(account) = carrier_wechat_oa::session::load_account(&home, bot_id) else {
                tracing::warn!(job = %job_name, bot_id,
                    "PublishPoll: no senders session, dropping pending ids (cannot resolve credentials)");
                for pid in &pending_ids {
                    let _ = carrier_wechat_oa::publish_tracker::remove(&home, bot_id, pid);
                }
                kernel.cron_scheduler.record_success(job_id);
                return Ok(());
            };
            let http = reqwest::Client::new();
            let token = match carrier_wechat_oa::token::get_token(
                &http,
                &account.app_id,
                &account.app_secret,
            )
            ...
            for pid in &pending_ids {
                match carrier_wechat_oa::api::freepublish_get(&http, &token, pid).await {
                    Ok(v) => {
        ...
        carrier_types::scheduler::CronAction::CommentPull { channel: _, bot_id } => {
            ...
            let articles =
                match carrier_wechat_oa::api::freepublish_batchget(&http, &token, 0, 20, true).await {
                    Ok(v) => v,
                    Err(e) => {
            ...
            for item in articles["item"].as_array().unwrap_or(&Vec::new()) {
                ...
                let Some(mid) = carrier_wechat_oa::api::extract_mid_from_url(url) else {
                    continue;
                };
                let title = truncate_chars(news["title"].as_str().unwrap_or("（无标题）"), 30);
                let comments =
                    match carrier_wechat_oa::api::comment_list(&http, &token, mid, 0, 0, 0, 50).await {
                        Ok(v) => v,
                        Err(e) => {
                ...
                let all_ids: Vec<i64> = raw
                    ...
                    .collect();
                let new_ids =
                    carrier_wechat_oa::comment_state::filter_new(&home, bot_id, &mid.to_string(), &all_ids);
                if new_ids.is_empty() {
                    continue;
                }
                ...
                if let Err(e) =
                    carrier_wechat_oa::comment_state::mark_seen(&home, bot_id, &mid.to_string(), &new_ids)
                {
...
async fn wechat_oa_official_total(kernel: &Arc<CarrierKernel>, app_id: &str) -> Option<i64> {
    let account = carrier_wechat_oa::session::load_account(&kernel.config.home_dir, app_id)?;
    let http = reqwest::Client::new();
    let token = carrier_wechat_oa::token::get_token(&http, &account.app_id, &account.app_secret)
        .await
        .map_err(|e| {
        ...
        .ok()?;
    match carrier_wechat_oa::api::user_get(&http, &token, None).await {
        Ok(r) => Some(r.total),
        Err(e) => {
...
fn build_follower_report(
    agent_name: &str,
    since: &str,
    stats: &carrier_memory::follower_store::FollowerStats,
...
impl CarrierKernel {
    ...
    fn reconcile_publish_poll(&self) {
        ...
        let accounts = carrier_wechat_oa::publish_tracker::pending_accounts(home);
        if accounts.is_empty() {
            return;
        }
        let mut changed = false;
        for app_id in accounts {
            let Some(session) = carrier_wechat_oa::session::load_account(home, &app_id) else {
                continue;
            };
            ...
            let exists = self.cron_scheduler.list_jobs(agent_id).into_iter().any(|j| {
                matches!(
                    &j.action,
                    carrier_types::scheduler::CronAction::PublishPoll { bot_id, .. } if bot_id == &app_id
                )
            });
            if exists {
                continue;
            ...
            let job = carrier_types::scheduler::CronJob {
                ...
                enabled: true,
                schedule: carrier_types::scheduler::CronSchedule::Every { every_secs: 1800 },
                action: carrier_types::scheduler::CronAction::PublishPoll {
...
mod tests {
    ...
    fn follower_report_text_carries_official_total() {
        ...
        let stats = carrier_memory::follower_store::FollowerStats {
            new_followers: 12,
            unfollows: 2,
            active: 1095,
            pushable: 40,
