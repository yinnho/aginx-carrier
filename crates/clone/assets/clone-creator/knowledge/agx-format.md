...
v3 核心原则：**源码格式 = 运行时格式**，解压即 workspace，无转换、无丢失。
...
│       └── flow.md
├── agents/             # 子代理目录（可选）
│   ├── code-reviewer.md  # 简单子代理（扁平）
│   └── architect/        # 复杂子代理（目录格式）
│       ├── AGENT.md
│       └── scripts/
│           └── analyze.toml
└── style/              # 风格样本（可选）
...
**不包含**: output/、sessions/、history/、agent.toml、AGENT.json（运行时数据，不属于分身身份）。
...
- `mcp_servers`（可选）：字符串数组，声明该分身依赖的 MCP 服务器 ID。安装时系统会连接对应的 MCP 服务器。
- `plugins`（可选）：字符串数组，声明该分身依赖的插件。值对应 `~/.opencarrier/plugins/` 下的目录名（不含 `opencarrier-plugin-` 前缀）。
...
此文件安装后由系统自动维护，不需要手动更新。

### EVOLUTION.md — 进化策略（推荐）

每个分身的学习配置。YAML frontmatter 供系统读取，规则段落注入到 prompt 供 LLM 遵循。
...
**双用途**：
- YAML frontmatter（`---` 之间）→ 系统配置，不注入 prompt
- frontmatter 之后的规则段落 → 注入 prompt，LLM 知道何时触发进化行为

**缺失时**: 使用默认配置（conservative 模式，200 文件上限，50MB 容量）。

### knowledge/ — 知识文件

**简单知识**：`knowledge/<topic>.md`（双层格式）
...
|------|------|------|
| `name` | 知识标题 | 自由文本 |
| `source` | 来源 | `manual` / `evolution` / `import` / `conversation` |
| `description` | 简短描述 | 一句话概括内容 |
...
核心流程概述（< 2000 字）。详细参考放 references/，示例放 examples/。
...
可选子目录：
- `flows/<name>/references/` — 详细参考文档，flow 激活时按需注入
- `flows/<name>/examples/` — 触发示例，帮助 agent 判断何时使用此 flow
- `flows/<name>/scripts/` — 可执行脚本，flow 运行时可通过 `shell_exec` 调用

scripts/*.toml 定义 HTTP API 调用：
...
AGENT.md 格式同上。scripts/*.toml 定义子代理专用的 API 调用，格式与 flow scripts 相同。
...
**与 flows 的区别**：Flows = "做什么"（操作手册），Agents = "谁来做"（执行实体）。
...
v3 下安装即解压，系统会自动：
1. 解压 .agx 到 `~/.opencarrier/workspaces/{name}/`
2. 从 template.json + 用户设置生成 agent.toml
3. 创建 AGENT.json（注册信息）
4. 创建运行时目录（output/、sessions/、history/、logs/）
5. 通过 API 注册 agent
6. 启动 agent
